...
class GAN():
    ...
    def get_non_batik_dataset(self):
      DATASET_FOLDER = "dtd_images"
      SHAPE = (128, 128,)
      dataset = []
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        X_non_batik = self.get_non_batik_dataset()

        # Rescale -1 to 1
        X_train = X_train / 127.5 - 1.
        X_non_batik = X_non_batik / 127.5 - 1.
        # X_train = np.expand_dims(X_train, axis=3)

        # Adversarial ground truths
        valid = np.concatenate((np.ones((batch_size // 2, 1)), np.zeros((batch_size // 2, 1))))
        ...
        for epoch in range(epochs):
            ...
            imgs = X_train[idx]
            non_batik = X_non_batik[idx]
            imgs = np.concatenate((imgs, non_batik))
            
            # Select random false image
